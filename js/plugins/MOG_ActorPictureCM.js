...
Moghunter.actor_fileName = String(
  Moghunter.parameters["File Name"] || "Actor_"
);
Moghunter.actor_cm1_visible = String(
  Moghunter.parameters["Face Visible"] || "true"
);
Moghunter.actor_cm1_slideX = Number(
  Moghunter.parameters["Face Slide X"] || 150
);
...
Moghunter.actor_cm2_visible = String(
  Moghunter.parameters["Bust Visible"] || "true"
);
Moghunter.actor_cm2_slideX = Number(
  Moghunter.parameters["Bust Slide X"] || -150
);
...
Moghunter.actor_cm2_breathEffect = String(
  Moghunter.parameters["Breath Effect"] || "true"
);
...
Game_Interpreter.prototype.pluginCommand = function (command, args) {
  ...
  if (command === "actorCM_fileName") {
    this.setActorCM(args);
  }
  if (command === "actorCM_breathEffect") {
    this.setActorBreathEffect(args);
  }
  return true;
};
...
Game_Interpreter.prototype.setActorBreathEffect = function (args) {
  ...
  for (var i = 0; i < $gameParty.members().length; i++) {
    ...
    if (actor._actorId === id) {
      if (actor) {
        actor.setActorCMbreathEffect(enable);
      }
    }
  }
};
...
Game_Actor.prototype.initMembers = function () {
  ...
  var breathEffect =
    String(Moghunter.actor_cm2_breathEffect) == "true" ? true : false;
...
//==============================
var _mog_actorPicCM_sbattle_createSpriteset =
...
Actor_CMPicture.prototype.isVisible = function () {
  if (!$gameSystem._actorCmOffSet[0]) {
    return false;
  }
  if (!$gameTemp._actorCmData[2]) {
    return false;
  }
...
Actor_CMPicture.prototype.refreshSprites = function () {
  ...
  if (this._supSprite) {
    this.refreshBitmapCM1();
  }
  if (this._bust) {
    this.refreshBitmapCM2();
  }
};
...
Actor_CMPicture.prototype.needRefreshActor = function () {
  if ($gameTemp._actorCmData[1]) {
    return true;
  }
  if (BattleManager.actor() != this._actor) {
    return true;
  }
  if (this._actor && this._CmName != this._actor.battleBust_1()) return false;
};
...
Actor_CMPicture.prototype.getDataCM1 = function () {
  this._supSpriteData = [
    this._supSprite.bitmap.width,
    this._supSprite.bitmap.height,
    Moghunter.actor_cm1_x,
    Moghunter.actor_cm1_y,
    0,
    0,
    true,
    Moghunter.actor_cm1_slideX,
    Moghunter.actor_cm1_slideY,
  ];
  this._supSprite.visible = this._actor._actorCMData[6];
  this._supSprite.opacity = 0;
  this._supSprite.x = this._supSpriteData[6]
    ? this._supSpriteData[2] + this._supSpriteData[7]
    : this._supSpriteData[2];
  this._supSprite.y = this._supSpriteData[6]
    ? this._supSpriteData[3] + this._supSpriteData[8]
    : this._supSpriteData[3];
};
...
Actor_CMPicture.prototype.needGetData1 = function () {
  ...
  if (this._supSpriteData) {
    return false;
  }
  if (!this._supSprite.bitmap) {
    return false;
  }
  if (!this._supSprite.bitmap.isReady()) {
    return false;
  }
  return true;
};
...
Actor_CMPicture.prototype.getDataCM2 = function () {
  this._bustData = [
    this._bust.bitmap.width,
    this._bust.bitmap.height,
    Moghunter.actor_cm2_x,
    Moghunter.actor_cm2_y,
    0,
    0,
    true,
    Moghunter.actor_cm2_slideX,
    Moghunter.actor_cm2_slideY,
    0,
    0,
    0,
  ];
  ...
  this._bust.x = this._bustData[6]
    ? this._bustData[2] + this._bustData[7]
    : this._bustData[2];
  this._bust.y = this._bustData[6]
    ? this._bustData[3] + this._bustData[8]
    : this._bustData[3];
...
Actor_CMPicture.prototype.updateCM2 = function () {
  ...
  if (this._breahEffect && this._bust.opacity > 0) {
...
Actor_CMPicture.prototype.updateBreathEffect = function () {
  ...
  this._bust.scale.y = 1.0 + this._bust.zoom.y;
};
...
Actor_CMPicture.prototype.needGetData2 = function () {
  ...
  if (!this._bust.bitmap) {
    return false;
  }
  if (!this._bust.bitmap.isReady()) {
    return false;
  }
  return true;
};
...
Actor_CMPicture.prototype.update = function () {
  ...
  if (this.needRefreshActor()) {
    this.refreshActor();
  }
  if (this.needGetData1()) {
    this.getDataCM1();
  }
  if (this.needGetData2()) {
    this.getDataCM2();
  }
  if (this._supSpriteData) {
    this.updateCM1();
